...
import { ListSubheader, Avatar, Collapse } from '@material-ui/core';
...
import CustomScrollbars from './../../components/AppLayout/CustomScrollbars';
...
function SideBar(props) {
  ...
  const handleDrawerOpen = () => {
    setOpen(true);    
    document.querySelectorAll("#nested-list-subheader").forEach(ele => {return ele.style.opacity = 1})
  ...
  const handleDrawerClose = () => {
    setOpen(false);
    document.querySelectorAll("#nested-list-subheader").forEach(ele => {return ele.style.opacity = 0})
      ...
      >
        <CustomScrollbars className=" scrollbar">
          ...
          <Divider />
          {
            ...
            Object.keys(props.sideBar).map((ele,index) => {
              return (<React.Fragment key={index}>
                <List
                  
                  subheader={
                    <ListSubheader style={{opacity:"0"}} component="div" id="nested-list-subheader">
                      { ele }
                    </ListSubheader>
                  }
                >
                  {
                    props.sideBar[ele].map((item,index) => {
                      
                      return (
                        ((item.component != "Collapse") ? (<ListItem key={index} button component={item.component} to={item.to} state={item.state} >
                          <ListItemIcon>
                            { item.icon }
                          </ListItemIcon>
                              ...
                              <ListItem button >
                                ...
                                <Collapse in={open} timeout="auto" unmountOnExit>
                                  <List component="div" disablePadding>
                                    {
                                      item.child.map((ele1, index1) => {
                                        return <ListItem className={classes.nested} key={index1} button component={ele1.component} to={ele1.to} state={ele1.state} >
                                                  <ListItemIcon>
                                                    {ele1.icon}
                                                  </ListItemIcon>
                                                  <ListItemText primary={ele1.label} />
                                                </ListItem>
                                      })
                                    }
                                  </List>
