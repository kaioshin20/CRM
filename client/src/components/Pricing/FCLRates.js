...
import { FCL_Rates } from './AddRates/ExcelTemplates';
...
export default function FCLRates() {

  const classes = useStyles();
  const pricingState = useSelector(state => state.pricing)
  let location = useLocation();
  const [template, setTemplate] = React.useState(FCL_Rates)
  
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getAllRates({type: FCL_Rates.name}))
    // if(location.state === "ICDRates"){
    //   setTemplate(FCL_Rates)
    // }
  }, [])
                    ...
                    <Grid container justify="space-between">
                        <Grid item xs={12} sm={4} style={{alignSelf:"center"}}>
                            <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb" className={classes.ContainerBreadcrumb}>
                                ...
                                <Typography color="textPrimary">FCL Buy Rates</Typography>
                            </Breadcrumbs>
                        </Grid>
