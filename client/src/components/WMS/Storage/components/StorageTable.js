...
const headCells = [
  ...
  { id: 'storage_no', numeric: false, disablePadding: false, label: 'Storage No.' },
  { id: 'created', numeric: false, disablePadding: false, label: 'Date Of Creation' },
...
export default function StorageTable({Storage_rows}) {
...
if(filterType === 4){
      ...
      return Storage_rows.filter((row) => { return row._id.search(keyword) > -1 || (row.product_name).search(keyword) > -1 || (row.category).search(keyword) > -1 || (row.storage_no).search(keyword) > -1 || (row.created).search(keyword) > -1  })
    }
  }

  console.log("in booking table",Storage_rows)
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.storage_no}</TableCell>
