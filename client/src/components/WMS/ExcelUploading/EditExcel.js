...
handleFormSubmit = (newData, openType) => {
  ...
  render() {
	...
	let columns = ICD_Rates.columns.map(column => {
		return {
			label: column.label,
            name: column.name,
            options: {
                ...
                customBodyRender: (value, tableMeta, updateValue) => (
					// <React.Fragment>
						...
						onChange={event => {
							...
							var temp_errors = Excel_Data_Validation(tempData, ICD_Rates)
							this.setState({
								...this.state,
								errors: temp_errors
							})
				...
				setCellProps: (value, i, j) => {
					
					
					// console.log(error, bgColor)		
 ...
 const options = {
    ...
    return (
		<React.Fragment>
            ...
            <MUIDataTable title={ICD_Rates.label} data={this.state.data} columns={columns}  options={options}/>

		{this.state.formOpen && <AddNew open={this.state.formOpen} preData={this.state.dataToPass} openType={this.state.openType} onClose={this.handleFormClose} handleFormSubmit ={(data, openType) => this.handleFormSubmit(data, openType) }  />}
        </React.Fragment>
    );
