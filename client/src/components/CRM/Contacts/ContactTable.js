...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, selected, setSelected, deleteContacts, openModal } = props;
      ...
      {numSelected > 0 ? (
        ...
        <ButtonGroup className={classes.title} color="primary" aria-label="outlined primary button group">
            <Button onClick={() => {
              ...
              }} variant={(props.filterType === 1 || props.filterType === 4) ? "contained" : "outlined"}>All Contacts</Button>
            <Button onClick={() => {
                props.setFilterType(2)
              }} variant={(props.filterType === 2) ? "contained" : "outlined"}>My Contacts</Button>
            <Button onClick={() => {
                props.setFilterType(3)
              }} variant={(props.filterType === 3) ? "contained" : "outlined"}>Unassigned Contacts</Button>
        </ButtonGroup>
...
export default function EnhancedTable(props) {
  ...
  const {rows, loading, deleteContacts} = props
  ...
  const filterRows = () => {
    if(filterType===1){
      ...
      return rows.filter((row) => { return row._id.search(keyword) > -1 || (row.fname).search(keyword) > -1 || row.email.search(keyword) > -1 || row.mobile.search(keyword) > -1 })
    }
  ...
  return (
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        <EnhancedTableToolbar numSelected={selected.length} selected={selected} setSelected={setSelected} deleteContacts={deleteContacts} filterType={filterType} setFilterType={setFilterType} filterRows={filterRows} findSearch={findSearch} handleDeleteRows={handleDeleteRows} openModal={openModal} />
        <TableContainer>
          <Table
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell component="th" id={labelId} scope="row" padding="none" >
                        <Link to={{pathname:'/crm/contact/id/', state: row._id}} > {row.fname + ' ' + row.lname} </Link>
                      </TableCell>
                      <TableCell align="right" >{row.email}</TableCell>
                      <TableCell align="right" >{row.mobile}</TableCell>
                      <TableCell align="right" >{row.leadstage}</TableCell>
      ...
      { open && <CreateContactModal type={modalType} data={rows.filter(contact => {return contact._id == selected[0] })[0]} open={open} onClose={() => onClose()} />}
    </div>
  );
}
